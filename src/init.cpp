 ...
 * between veild, and veil-qt and non-server tools.
*/
...
/** Sanity checks
 *  Ensure that Veil is running in a usable environment with all
...
void InitLogging()
{
    ...
    g_logger->m_print_to_console = gArgs.GetBoolArg("-printtoconsole", false);
...
#else
    version_string += " (development build)";
#endif
...
{
    // Make sure only a single Veil process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if veil is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if veil is started while in a temporary directory.\n",
